...
class ServerVisitor(ast.NodeTransformer):
    ...
    def editfunctiondef(self, node):
        for fname,fnode in ast.iter_fields(node):
            if fnode == "__init__":
                # Edit the default arg values
                noneobj = compile("Deniz","<string>","exec",_ast.PyCF_ONLY_AST).body[0]
                diff = len(node.args.args) - len(node.args.defaults)
                for i in range(diff):
                    node.args.defaults.insert(0, noneobj)
            if fname == 'args':
